        ...
        <form method="GET" action="{{ route('users.index') }}" class="space-y-4 sm:space-y-0 sm:flex sm:items-end sm:space-x-4">
            ...
            <div>
                ...
                <select id="is_admin" 
                    ...
                    <option value="0" {{ request('is_admin') === '0' ? 'selected' : '' }}>Usuário Padrão</option>
                </select>
            </div>
                        ...
                        <div class="flex space-x-2">
                            @if($user->id !== auth()->id())
                                ...
                                <button onclick="toggleUserStatus({{ $user->id }}, {{ $user->is_active ? 'false' : 'true' }})" 
                                        class="{{ $user->is_active ? 'text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300' : 'text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300' }}" 
                                ...
                                <!-- Delete -->
                                <form method="POST" action="{{ route('users.destroy', $user) }}" class="inline" onsubmit="return confirm('Tem certeza que deseja deletar este usuário?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
